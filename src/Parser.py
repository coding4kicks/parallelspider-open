...
class Parser:
    ...
    def run(self, doc):
        ...
        try:
            ...
            # http://lxml.de/
            out = BeautifulSoup(doc, "lxml")

            #output = {}
        
            
            # Find content for specified tags
            for tag in out.find_all(self.tag_list):
                self.output[tag] = tag.text.encode('utf-8') 
            ...
            self.link_list = __process_links(links, self.site_url)
...
def __process_links(links, site):
    """ 
