...
class TestPathFiltering(unittest.TestCase):
    """Tests Mr Feynman's ability to filter paths to follow."""
    ...
    def setUp(self):
        ...
        config = _get_config(test) 
        self.brain = Brain(self.params['test_site'], config)
        test_path = ('{0}/{1}_results_paths_links').format(
            _test_results_dir(), self.params['test_file'])
        with open(test_path) as f:
            self.results = f.read()

    def test_paths(self):
        """Test correct link on site link filtering"""
        _analyze(self.brain, self.params)
        self.assertEqual(self.results, str(self.brain.on_site_links))
...
###############################################################################
def generate_output(test_type, output_type=None):
    """
    ...
    Paths test type generates links vice map or reduce output.
    """
    ...
    if test_type == 'paths':
        test_file = ('{0}/{1}_results_{2}_links').format(
            _test_results_dir(), params['test_file'], test_type)
        with open(test_file, 'w') as f:
            _analyze(brain, params)
...
class _FakeRobotText(object):
    ...
    def get(self):
        """Robotparser object of real robot.txt file, currently for foxnews."""
    ...
    def get_empty(self):
        """Robotparser object of empty robot.txt file."""
